 ...
 *
 * WHAT THIS PAGE DOES:
 *   Displays an interactive Leaflet.js map + partner directory cards.
 *   Map pins are color-coded by organization type:
 *     Navy   = National Partner
 *     Amber  = Local Church
 *     Teal   = Organizing Group
 *   Clicking a partner card syncs the map to show that organization's pin.
 *
 * HOW TO MANAGE CONTENT (no code editing required):
 *   - Add/edit partners: Faithful Witness → Network Partners → Add New
 *   - Required for map pin: fill in Latitude + Longitude in the details box
 *     (right-click any location on Google Maps to copy coordinates)
 *   - Organization Type controls the pin color
 *   - Partners without lat/lng appear in the list but NOT on the map
 *
 * DO NOT EDIT THIS FILE to change partner content. Only edit for layout changes.
 */

get_header();

// Page-level ACF fields — edit via Pages → Find Your Network → edit
$hero_eyebrow  = get_field( 'fw_page_hero_eyebrow' )  ?: __( 'Community Network', 'faithfulwitness' );
$hero_title    = get_field( 'fw_page_hero_title' )    ?: __( 'You are not alone. Find faithful witnesses near you.', 'faithfulwitness' );
$hero_subtitle = get_field( 'fw_page_hero_subtitle' ) ?: __( 'Local organizing groups and partner organizations are working across the country. Connect, learn, and build together.', 'faithfulwitness' );
?>
